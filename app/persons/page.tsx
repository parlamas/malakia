...
export default function PersonsDirectoryPage() {
  ...
  useEffect(() => {
    setLoading(true);
    const t = setTimeout(() => {
      fetch(`/api/persons${query ? `?q=${encodeURIComponent(query)}` : ''}`)
        .then((r) => r.json())
        .then((data) => {
        ...
        {results.map((p) => (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              <span style={{ fontFamily: 'Georgia, serif', fontSize: 17, color: '#1C2024' }}>{p.displayName}</span>
              <span style={{ fontSize: 12, color: '#5F5E5A' }}>{p._count.posts} record{p._count.posts !== 1 ? 's' : ''}</span>
            </div>
            <p style={{ fontSize: 13, color: '#5F5E5A', margin: '4px 0 0' }}>
              {PERSONA_LABELS[p.personaCategory] ?? p.personaCategory} — {p.roleTitle}, {p.country}
            </p>
          </Link>
        ))}
      </div>
